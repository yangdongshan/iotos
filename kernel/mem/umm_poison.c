...
static void dump_mem ( const unsigned char *ptr, size_t len ) {
  while (len--) {
    UMM_ERROR(" 0x%.2x", (unsigned int)(*ptr++));
  }
}
...
static int check_poison( const unsigned char *ptr, size_t poison_size,
  ...
  if (!ok) {
    UMM_ERROR( "No poison %s block at: 0x%lx, actual data:", where, (unsigned long)ptr);
...
static int check_poison_block( umm_block *pblock ) {
  ...
  if (pblock->header.used.next & UMM_FREELIST_MASK) {
    UMM_ERROR( "check_poison_block is called for free block 0x%lx\n", (unsigned long)pblock);
  } else {
...
int umm_poison_check(void) {
  ...
  if (umm_heap == NULL) {
      kdebug_print("heap is not initialzied\r\n");
      //umm_init();
  }
