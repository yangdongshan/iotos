...
int umm_integrity_check(void) {
  ...
  while(1) {
    ...
    if (cur >= UMM_NUMBLOCKS) {
        kdebug_print("heap integrity broken: too large next free num: %d "
    ...
    if (UMM_PFREE(cur) != prev) {
      kdebug_print("heap integrity broken: free links don't match: "
          "%d -> %d, but %d -> %d\n",
  ...
  while(1) {
    ...
    if (cur >= UMM_NUMBLOCKS) {
      kdebug_print("heap integrity broken: too large next block num: %d "
          "(in block %d, addr 0x%lx)\n", cur, prev,
          (unsigned long)&UMM_NBLOCK(prev));
      ok = 0;
      goto clean;
    ...
    if ((UMM_NBLOCK(cur) & UMM_FREELIST_MASK)
        != (UMM_PBLOCK(cur) & UMM_FREELIST_MASK))
    {
      kdebug_print("heap integrity broken: mask wrong at addr 0x%lx: n=0x%x, p=0x%x\n",
    ...
    if (cur <= prev ) {
     kdebug_print("heap integrity broken: next block %d is before prev this one "
          "(in block %d, addr 0x%lx)\n", cur, prev,
          (unsigned long)&UMM_NBLOCK(prev));
      ok = 0;
      goto clean;
...
/* unmark */
    ...
    if (UMM_PBLOCK(cur) != prev) {
      kdebug_print("heap integrity broken: block links don't match: "
          "%d -> %d, but %d -> %d\n",
