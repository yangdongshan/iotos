...
static void umm_assimilate_up( unsigned short int c ) {
  ...
  if( UMM_NBLOCK(UMM_NBLOCK(c)) & UMM_FREELIST_MASK ) {
    ...
    UMM_DEBUG( "Assimilate up to next block, which is FREE\n" );
...
void umm_free( void *ptr )
{
  ...
  if( (void *)0 == ptr ) {
    UMM_DEBUG( "free a null pointer -> do nothing\n" );
  ...
  if( UMM_NBLOCK(UMM_PBLOCK(c)) & UMM_FREELIST_MASK ) {
    ...
    UMM_DEBUG( "Assimilate down to next block, which is FREE\n" );
    ...
    UMM_DEBUG( "Just add to head of free list\n" );
...
void *umm_malloc( size_t size )
{
  ...
  if (umm_heap == NULL) {
      kdebug_print("heap is not initialized\r\n");
      //umm_init();
  }
  ...
  if( 0 == size ) {
    UMM_DEBUG("malloc a block of 0 bytes -> do nothing\n" );
  ...
  while( cf ) {
    ...
    UMM_TRACE("Looking at block %6i size %6i\n", cf, blockSize );
...
#  error "No UMM_*_FIT is defined - check umm_malloc_cfg.h"
  ...
  if( UMM_NBLOCK(cf) & UMM_BLOCKNO_MASK && blockSize >= blocks ) {
    ...
    if( blockSize == blocks ) {
      ...
      UMM_DEBUG("Allocating %6i blocks starting at %6i - exact\n", blocks, cf );
      ...
      UMM_DEBUG("Allocating %6i blocks starting at %6i - existing\n", blocks, cf );
    ...
    UMM_DEBUG("Can't allocate %5i blocks\n", blocks );
...
void *umm_realloc(void *ptr, size_t size)
{
  ...
  if( ((void *)NULL == ptr) ) {
    UMM_DEBUG("realloc the NULL pointer - call malloc()\n" );
  ...
  if( 0 == size ) {
    UMM_DEBUG("realloc to 0 size, just free the block\n" );
  ...
  UMM_DEBUG("realloc blocks %i blockSize %i nextBlockSize %i prevBlockSize %i\n", blocks, blockSize, nextBlockSize, prevBlockSize );
   ...
   */
    ...
    if (blockSize >= blocks) {
        UMM_DEBUG( "realloc the same or smaller size block - %i, do nothing\n", blocks );
        /* This space intentionally left blank */
    } else if ((blockSize + nextBlockSize) >= blocks) {
        UMM_DEBUG( "realloc using next block - %i\n", blocks );
        umm_assimilate_up( c );
        blockSize += nextBlockSize;
    } else if ((prevBlockSize + blockSize) >= blocks) {
        UMM_DEBUG( "realloc using prev block - %i\n", blocks );
        ...
        UMM_DEBUG( "realloc using prev and next block - %i\n", blocks );
        umm_assimilate_up( c );
        umm_disconnect_from_free_list( UMM_PBLOCK(c) );
        c = umm_assimilate_down(c, 0);
        memmove( (void *)&UMM_DATA(c), ptr, curSize );
        ...
        UMM_DEBUG( "realloc a completely new block %i\n", blocks );
        void *oldptr = ptr;
        if( (ptr = umm_malloc( size )) ) {
            UMM_DEBUG( "realloc %i to a bigger block %i, copy, and free the old\n", blockSize, blocks );
            memcpy( ptr, oldptr, curSize );
            umm_free( oldptr );
        } else {
            UMM_DEBUG( "realloc %i to a bigger block %i failed - return NULL and leave the old block!\n", blockSize, blocks );
    ...
    if (blockSize > blocks ) {
        UMM_DEBUG( "split and free %i blocks from %i\n", blocks, blockSize );
